...
`claimRevision` uses one Firestore transaction. It validates `bookId`, returns `complete` for complete records, returns `in_progress` when `claimExpiresAtMs > nowMs`, and otherwise creates/reclaims `pending` with the supplied token. `completeRevision` and `failRevision` transactionally compare `claimToken`; stale writers throw `CorpusImportError` with `409` and code `claim_lost`.

`saveChapters` uses deterministic chapter IDs and Firestore batches of at most 400 writes. `listChapters` reads only the completed revision's ordered `chapterIds` manifest. `ensureWorkspace` merge-writes the source-selected `documents_v2` or `documents_v3` record with `bookId`, `corpusRevisionId`, title, and source file.
