# SUP / BOOK.0 / NRG.0 / Knowledge Graph 시각화 구현

## 목적

미팅이나 데모에서 `SUP.*`, `BOOK.0`, `NRG.0`, Knowledge Graph의 역할을 말로만 설명하기 어렵기 때문에 Graph 탭에 발표용 시각화 계층을 추가했다.
...
- `NRG.0`은 `BOOK.0` memory를 reader-position-safe claim으로 어떻게 바꾸는가?
...
- Knowledge Graph, BOOK.0, NRG/Reader Support를 별도 product card로 보여준다.

설명 포인트:

- `SUP.0`은 story memory 원천이다.
- `BOOK.0`은 챕터별 `SUP.0`과 선택적 `ENT.3`를 모아 책 단위 memory를 만든다.
- `NRG.0`은 `BOOK.0`을 현재 독자 위치에서 안전하게 보여줄 수 있는 claim view로 해석한다.
- Knowledge Graph는 `ENT.3`와 `SUP.0`을 Firestore node/edge로 투영한다.
- `SUP.7`은 `NRG.0` claim을 `SupportUnit`으로 보강하고, 최종 독자 UI와 연결되는 지점이다.
...
- Reader support는 이 raw edge를 직접 보여주는 것이 아니라, `NRG.0` claim을 거쳐 `SUP.7`에서 작은 support unit으로 바뀐다.
...
- 향후에는 edge 클릭 시 `NRG.0` claim, `/api/support-context` 응답, Reader support unit까지 연결하는 drill-down이 필요하다.
