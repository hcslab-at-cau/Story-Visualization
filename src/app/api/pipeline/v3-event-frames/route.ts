...
import { buildV3EventFrames } from "@/lib/pipeline/v3-memory-frames"
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    const sceneCards = await loadStageResult<V3SceneSituationCardsArtifact>(
      docId,
      chapterId,
      runId,
      stageKey("MEM.1"),
    ...
    const result: V3EventFramesArtifact = buildV3EventFrames({
      docId,
      chapterId,
      parents,
      memoryContract,
    ...
    await saveStageResult(docId, chapterId, runId, stageKey("EVENT.2"), result, { source })
