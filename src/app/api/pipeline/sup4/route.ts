...
import { runSupportCharacterRelations } from "@/lib/pipeline/support"
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    const result = runSupportCharacterRelations(sharedLog, memoryLog, docId, chapterId, {
      ...parents,
      "SUP.0": memoryLog.run_id,
      "SUP.1": sharedLog.run_id,
    })

    await saveStageResult(docId, chapterId, runId, stageKey("SUP.4"), result)
