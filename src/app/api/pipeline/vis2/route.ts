...
import { runStageBlueprintExtraction } from "@/lib/pipeline/vis2"
import {
  attachLLMDebug,
  createLLMClient,
  errorResponse,
...
import type { GroundedSceneModel, ScenePackets, VisualGrounding } from "@/types/schema"
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    const visualLog = await loadStageResult<VisualGrounding>(docId, chapterId, runId, stageKey("VIS.1"))

    const llm = createLLMClient(body)
    const result = attachLLMDebug(
      await runStageBlueprintExtraction(
        packetLog,
        groundedLog,
        llm,
        docId,
    ...
    await saveStageResult(docId, chapterId, runId, stageKey("VIS.2"), result)
