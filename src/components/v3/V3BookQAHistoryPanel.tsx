...
import type { V3BookQAHistoryEntry } from "@/lib/v3-book-qa-history-types"
...
interface Props {
  ...
  onSelect: (entry: V3BookQAHistoryEntry) => void
  onDelete: (entry: V3BookQAHistoryEntry) => void
  onLoadMore: () => void
}
...
export default function V3BookQAHistoryPanel({
  entries,
  chapterTitles,
  selectedEntryId,
  loading,
  ...
  return (
    <aside
      aria-label="Book question history"
    ...
    >
      <div className="flex items-start justify-between gap-3 border-b border-zinc-200 px-4 py-3">
        <div className="min-w-0">
          <p className="text-xs font-semibold uppercase tracking-wide text-zinc-500">Book question history</p>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        {loading ? (
          <p className="px-4 py-10 text-center text-sm text-zinc-500">Loading book question history...</p>
        ) : entries.length === 0 ? (
          <div className="px-4 py-10 text-center">
            <p className="text-sm font-medium text-zinc-700">No saved book questions yet</p>
          ...
          <ul className="divide-y divide-zinc-200">
            {entries.map((entry) => {
              ...
              const readerChapter = chapterTitles[entry.reader_position.chapter_id] ?? entry.reader_position.chapter_id
              return (
                <li key={entry.entry_id} className={selected ? "bg-zinc-100" : "bg-white"}>
                  ...
                  >
                    <span className="flex items-center justify-between gap-2">
                      ...
                      <span className="max-w-[11rem] truncate font-mono text-[11px] text-zinc-500">
                        {readerChapter} / P{entry.reader_position.pid}
                      </span>
                    </span>
                  ...
                  <div className="flex justify-end px-3 pb-2">
                    <button
                      ...
                      aria-label={`Delete saved book question: ${entry.question}`}
                    >
